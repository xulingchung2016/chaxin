...
public class BaseActivity_FA extends FragmentActivity {
	 ...
	 */
	public void showMoreDialog(View view,boolean isShare){
		int[] lt = new int[2];
	...
	protected void handleOutmemoryError() {
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				Toast.makeText(BaseActivity_FA.this, "内存空间不足！", Toast.LENGTH_SHORT)
						.show();
				// finish();
			}
		});
	...
	protected void handleNetworkError() {
		...
		if (network_err_count < 3) {
			Toast.makeText(BaseActivity_FA.this, "网速好像不怎么给力啊！", Toast.LENGTH_SHORT)
					.show();
		} else if (network_err_count < 5) {
			Toast.makeText(BaseActivity_FA.this, "网速真的不给力！", Toast.LENGTH_SHORT)
					.show();
		} else {
			Toast.makeText(BaseActivity_FA.this, "唉，今天的网络怎么这么差劲！",
					Toast.LENGTH_SHORT).show();
		}
	}

	protected void handleMalformError() {
		Toast.makeText(BaseActivity_FA.this, "数据格式错误！", Toast.LENGTH_SHORT).show();
	}

	protected void handleFatalError() {
		Toast.makeText(BaseActivity_FA.this, "发生了一点意外，程序终止！", Toast.LENGTH_SHORT)
				.show();
		finish();
	}
	...
	public void pay(String subject, String body, String price, String OutTradeNo,final Handler mHandler,int type) 
	{
		...
		Runnable payRunnable = new Runnable()
		{
			...
			{
				// 构造PayTask 对象
				PayTask alipay = new PayTask(BaseActivity_FA.this);
				// 调用支付接口，获取支付结果
	...
	public void goOut() {
		...
		SharedPreferencesHelp.SavaString(BaseActivity_FA.this, "phoneNo", "");
		SharedPreferencesHelp.SavaString(BaseActivity_FA.this, "id", "");
		SharedPreferencesHelp.SavaString(BaseActivity_FA.this, "sex", "2");
		SharedPreferencesHelp.SavaString(BaseActivity_FA.this, "nickName", "");
		SharedPreferencesHelp.SavaString(BaseActivity_FA.this, "image", "");
		SharedPreferencesHelp.SavaBoolean(BaseActivity_FA.this, "isLogin", false);
